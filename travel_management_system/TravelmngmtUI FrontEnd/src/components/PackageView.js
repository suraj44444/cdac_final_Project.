    ...
    return (
        ...
        <div className="pkg-view">
          <div className="title-and-place">
           ...
           <div>
           <span className="pkg-price"><b>INR {props.data.cost}</b></span><br/>
           <span className="pkg-register-button"><Button variant="contained" onClick={()=>{handleRegister()}}>Package Book</Button></span> &nbsp;
           <span className="delete-pkg-btn" onClick={()=>{handleDelete()}}>{global.isuser ? <Button variant="outlined" color="error" size="small"><DeleteIcon /></Button> : ""}</span>
           </div>
           </div>
        </div>
    )
